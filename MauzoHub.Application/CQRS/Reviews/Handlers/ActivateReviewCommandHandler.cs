...
{
    public class ActivateReviewCommandHandler : IRequestHandler<ActivateReviewCommand, GetReviewDto>
    {
        private readonly IReviewRepository _reviewRepository;
        private readonly IHttpContextAccessor _httpContextAccessor;
        public ActivateReviewCommandHandler(IReviewRepository reviewRepository, IHttpContextAccessor httpContextAccessor)
        {
            _reviewRepository = reviewRepository;
            _httpContextAccessor = httpContextAccessor;
        }

        public async Task<GetReviewDto> Handle(ActivateReviewCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var reviewToActivate = await _reviewRepository.ActivateReview(request.Id);

                var reviewDto = new GetReviewDto
                {
                    Id = reviewToActivate.Id,
                    ProductOrServiceId = reviewToActivate.ProductOrServiceId,
                    UserId = reviewToActivate.UserId,
                    Rating = reviewToActivate.Rating,
                    Comment = reviewToActivate.Comment,
                };
